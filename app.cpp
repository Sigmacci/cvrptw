...
double grasp(Transport transport, Customers customers, int randomization_percentage)
{
    double best = numeric_limits<double>::max(), local_best;
    ...
    {
        cout << "iter: " << iter << endl;
        vector<vector<int>> instance = greedy_randomized(transport, customers, time_matrix, randomization_percentage);
...
vector<vector<int>> local_search(vector<vector<int>> solution, Transport transport, Customers customers, double **time_matrix, double *best)
{
    ...
    int iterations = 0;
    double best_cost = cost(new_solution, customers, time_matrix);
